...
dds <- DESeqDataSetFromMatrix(countData = data$count_data, 
                              colData = data$sample_data, design = ~group)
...
#Do not change:
heatmap.annotation.col <- data$sample_data %>%
...
counts.plot <- lapply(counts_to_plot, function(x) {
  plotCounts(dds, data$annotation_data[data$annotation_data$GeneSymbol == x,]$ENSG,
              intgroup = c("celltype", "condition", "time"), returnData = TRUE)
  }) %>%
  set_names(counts_to_plot) %>%
  lapply(rownames_to_column, var = "Sample_ID") %>%
  bind_rows(.id = "GeneSymbol") %>%
  group_by(GeneSymbol, celltype, condition, time) %>%
...
ggplot(counts.plot, aes(x = interaction(celltype, condition, sep=":", lex.order = T), y = mean_count, fill = time)) +
